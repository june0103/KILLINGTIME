        ...
        <table class="list-table">
          <thead>
            ...
            if(isset($_GET['page'])){
              ...
              $sql = mq("select * from drip order by idx desc limit  $start_num, $list"); // board테이블에있는 idx를 기준으로 내림차순해서 %%개까지 표시
                while($board = $sql->fetch_array())
                {
                  $sql3 = mq("select * from drip_rating where con_num='".$board['idx']."'");
                  $driplike = mysqli_num_rows($sql3);
                  $title=$board["title"];
                  if(mb_strlen($title,'utf-8')>30)
                  {
                    $title=str_replace($board["title"],mb_substr($board["title"],0,30,"utf-8")."...",$board["title"]); //title이 30을 넘어서면 ...표시
                  }
                  $sql2 = mq("select * from driprep where con_num='".$board['idx']."'"); //reply테이블에서 con_num이 board의 idx와 같은 것을 선택
