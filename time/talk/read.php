<?php
  ...
  if(isset($_POST['liked'])){
    $postid = $_POST['postid'];
    $result1 = mysqli_fetch_array(mq("select * from talk where idx='".$postid."'"));
    $n=$result1['likes'];
    $nn = $n+1;
    $n1= mq("update talk set likes='".$nn."' where idx='".$postid."' ");
    $n2 =mq("insert into talk_like(userid, postid) values('".$_SESSION['userid']."','".$postid."')");
    exit();
  }
  if(isset($_POST['unliked'])){
    ...
    $n=$result1['likes'];
    $nn = $n-1;
    $n2 =mq("delete from talk_like where postid='".$postid."' and userid='".$_SESSION['userid']."'");
    $n1= mq("update talk set likes='".$nn."' where idx='".$postid."' ");
...
<body>
  <?php
    ...
    if($_COOKIE['talk'.$bno.$_SESSION['name']] == false)
    {
      $hit = mysqli_fetch_array(mq("select * from talk where idx ='".$bno."'"));
      $hit = $hit['hit'] + 1;
      $fet = mq("update talk set hit = '".$hit."' where idx = '".$bno."'");
      $sql = mq("select * from talk where idx='".$bno."'"); /* 받아온 idx값을 선택 */
      $board = $sql->fetch_array();
      setcookie('talk'.$bno.$_SESSION['name'] , TRUE , time()+60*60*24, "/");
...
<div id="board_read">
  ...
  <h4>아무말이나 적어봐!!</h4>
	...
	<div id="bo_line"></div>
      ...
      <div id="post-info">
        <?php
        $sql2 = mq("select * from talk_like where userid='".$_SESSION['userid']."' and postid='".$board['idx']."' ");
		...
		<ul>
			...
			<?php
				...
				?>
					<li><a href="dripList.php"><button>목록</button></a></li>
				<?php }
        $sql12 = mq("select * from talkrep where con_num='".$board['idx']."'"); //reply테이블에서 con_num이 board의 idx와 같은 것을 선택
...
<h3>댓글목록[<?php echo $rep_count; ?>]</h3>
	<div id="bo_line"></div>
	<?php
		$sql3 = mq("select * from talkrep where con_num='".$bno."' order by idx desc");
