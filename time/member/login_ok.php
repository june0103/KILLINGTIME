...
<?php
	...
	if(password_verify($password, $hash_pw)) //만약 password변수와 hash_pw변수가 같다면 세션값을 저장하고 알림창을 띄운후 main.php파일로 넘어갑니다.
	{
		...
		setcookie('cook_userid' , $_POST["userid"] , time()+3600, "/");
		setcookie('cook_userid_input' , $_POST["userid"] , time()+3600, "/");
		setcookie('cook_userpw' , $_POST["userpw"] , time()+3600, "/");

		echo "<script>alert('로그인되었습니다.'); location.href='/time/index.php';</script>";
