<?php
  ...
  if(isset($_POST['liked'])){
    $postid = $_POST['postid'];
    $result1 = mysqli_fetch_array(mq("select * from video where idx='".$postid."'"));
    $n=$result1['likes'];
    $nn = $n+1;
    $n1= mq("update video set likes='".$nn."' where idx='".$postid."' ");
    $n2 =mq("insert into video_like(userid, postid) values('".$_SESSION['userid']."','".$postid."')");
    exit();
  }
  if(isset($_POST['unliked'])){
    ...
    $n=$result1['likes'];
    $nn = $n-1;
    $n2 =mq("delete from video_like where postid='".$postid."' and userid='".$_SESSION['userid']."'");
    $n1= mq("update video set likes='".$nn."' where idx='".$postid."' ");
...
<body>
    ...
    if($_COOKIE['video'.$bno.$_SESSION['name']] == false)
    {
      $hit = mysqli_fetch_array(mq("select * from video where idx ='".$bno."'"));
      $hit = $hit['hit'] + 1;
      $fet = mq("update video set hit = '".$hit."' where idx = '".$bno."'");
      $sql = mq("select * from video where idx='".$bno."'"); /* 받아온 idx값을 선택 */
      $board = $sql->fetch_array();
      setcookie('video'.$bno.$_SESSION['name'] , TRUE , time()+60*60*24, "/");
...
<div id="board_read">
  <h3 style="color:#509ee7;
    margin-top: 10px;
    margin-bottom: 20px;">꿀잼 영상 모음</h3>
  <h4>보다보면 시간삭제 되는 꿀잼 영상모음</h4>
	...
	<div id="bo_line"></div>
        ...
        <iframe id="ytplayer" type="text/html" width="500px" height="450px"
        src="https://www.youtube.com/embed/<?php echo $board['url'] ?>?rel=0&showinfo=0&color=white&iv_load_policy=3" frameborder="0" allowfullscreen>
      ...
      <div id="post-info">
        <?php
        $sql2 = mq("select * from video_like where userid='".$_SESSION['userid']."' and postid='".$board['idx']."' ");
		...
		<ul>
			...
			<?php
				...
				?>
					<li><a href="dripList.php"><button>목록</button></a></li>
				<?php }
        $sql12 = mq("select * from videorep where con_num='".$board['idx']."'"); //reply테이블에서 con_num이 board의 idx와 같은 것을 선택
...
<h3>댓글목록[<?php echo $rep_count; ?>]</h3>
	<div id="bo_line"></div>
	<?php
		$sql3 = mq("select * from videorep where con_num='".$bno."' order by idx desc");
